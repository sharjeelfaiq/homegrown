...
export default function StudioShell() {
  ...
  const voiceDeleteTimers = useRef(new Map<number, string | number>())
  useEffect(() => () => {
    for (const [timer, toastId] of voiceDeleteTimers.current) {
      window.clearTimeout(timer)
      toast.dismiss(toastId)
    }
    voiceDeleteTimers.current.clear()
  }, [])
  ...
  /** Optimistically remove the voice once the password modal submits; restore
   *  it at its original position if the server rejects the deletion. */
  async function handleDeletePreset(id: string, adminPassword: string) {
    const doomed = presets.find((p) => p.id === id)
    if (!doomed) return
    try {
      await verifyAdminPassword(adminPassword)
    } catch (e) {
      const message = e instanceof ApiError ? e.message : 'Admin password could not be verified'
      setError(message)
      throw e
    }
    ...
    const restoreVoice = () => {
      setPresets((prev) => prev.some((p) => p.id === id)
        ? prev
        : [...prev.slice(0, Math.min(index, prev.length)), doomed, ...prev.slice(Math.min(index, prev.length))])
  ...
  async function handleDeleteHistory(id: string, adminPassword: string) {
    ...
    try {
      await deleteHistoryEntry(id, adminPassword)
      refreshHistory()
    } catch (e) {
      for (const [key, page] of snapshots) historyQueryClient.setQueryData(key, page)
      const message = e instanceof ApiError ? e.message : 'Failed to delete voiceover'
      setError(message)
  ...
  return (
    ...
    >
      ...
      {/* Toasts. `theme` comes from OUR nine-theme id, not sonner's default
          ...
          useJobToasts). Everything else dismisses itself, so a second dismiss
          control would be noise. Sonner's default also hangs it 6px
