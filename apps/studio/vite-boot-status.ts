 ...
 *
 * `services/voice-api/main.py`'s `lifespan()` publishes its phases to
 * `services/voice-api/storage/boot_status.json` regardless of whether it is frozen, so in
...
function storageDir(): string {
  // Mirror services/voice-api/main.py's dev branch: STORAGE_DIR = services/voice-api/storage, with
  ...
  return resolve(ROOT, '..', '..', 'services', 'voice-api', 'storage')
}
