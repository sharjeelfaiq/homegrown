...
Build from the services/voice-api/ folder:
...
BACKEND_DIR = REPO_ROOT / "services" / "voice-api"
...
datas += [(str(REPO_ROOT / "engine" / "qwen"), "qwen")]

# Built Studio (must exist -- run `npm run build` in apps/studio/ first).
frontend_dist = REPO_ROOT / "apps" / "studio" / "dist"
if not frontend_dist.is_dir() or not (frontend_dist / "index.html").exists():
    raise SystemExit(
        "apps/studio/dist is missing or incomplete -- run `npm run build` in apps/studio/ before building backend.spec"
    )
datas += [(str(frontend_dist), "frontend_dist")]

a = Analysis(
    ["run.py"],
    pathex=[str(BACKEND_DIR), str(REPO_ROOT / "engine")],
    binaries=binaries,
    datas=datas,
