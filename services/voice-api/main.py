...
if not getattr(sys, "frozen", False):
    # Dev mode: add the dedicated engine import root containing the vendored
    # `qwen` package. Frozen builds ship it at the bundle root instead.
...
DEFAULT_ADMIN_PASSWORD = "Homegrown-Admin-8731!"
...
def _require_admin_password(password: str) -> None:
    ...
    if not hmac.compare_digest(password, expected):
        raise HTTPException(403, "Incorrect admin password; nothing was deleted.")
...
# Overridable via MODEL_PATH in services/voice-api/.env -- the default below only holds
# on the original dev machine's local model cache. Any other host (including
# a separate environment) must set MODEL_PATH to wherever it downloaded the snapshot.
...
# ../../engine/qwen/HOW_TO_RUN.md). Empirically tested: 800 chars / 700 max_new_tokens
...
# Generation time is REPORTED, never predicted. There was an estimator here --
# _JOB_OVERHEAD_S + chunks * median(seconds per chunk), scored at 22% mean error
# against history.json -- and it was retired on 2026-09-13 because a median is
# beaten by half of all jobs by construction and the measured overrun was worse
# than that: 12 of 12 jobs on one batch ran over, by 1.2x to 2.5x, none under.
#
# Chunk resampling is why the tail is so fat: _chunk_duration_is_sane resamples
# a degenerate chunk up to CHUNK_ATTEMPTS times, and a median deliberately
# discards exactly those runs (measured p90 32.4s/chunk against p50 19.3s).
#
# generation_s and total_chunks are still recorded on every history entry, so
# the raw material survives if anyone wants to try again. The full
# model-selection table is in docs/gpu-notes.md -- read it before proposing a
# replacement.
...
def _migrate_preset_media_paths() -> None:
    """Canonicalize valid legacy preset paths once, preserving a backup."""
    changed = False
    for preset in _presets:
        raw = preset.get("audio_path")
        try:
            resolved = resolve_stored_media_path(raw, STORAGE_DIR)
            canonical = storage_relative_path(resolved, STORAGE_DIR)
        except StoredMediaPathError as exc:
            logger.warning("Preset %s has unresolved reference media: %s", preset.get("id"), exc)
            continue
        if raw != canonical:
            preset["audio_path"] = canonical
            changed = True
            logger.info("Migrated preset %s reference path", preset.get("id"))
    if changed:
        stamp = time.strftime("%Y%m%d-%H%M%S")
        backup = PRESETS_FILE.with_name(f"{PRESETS_FILE.name}.migration-{stamp}.bak")
        shutil.copy2(PRESETS_FILE, backup)
        _save_json(PRESETS_FILE, _presets)
        logger.info("Backed up presets to %s before path migration", backup)


_migrate_preset_media_paths()


def _reference_path(preset: dict) -> Path:
    """Resolve a preset clip and persist a valid legacy-path migration."""
    previous = preset.get("audio_path", "")
    resolved = resolve_stored_media_path(previous, STORAGE_DIR)
    canonical = storage_relative_path(resolved, STORAGE_DIR)
    if previous != canonical:
        with _store_lock:
            stamp = time.strftime("%Y%m%d-%H%M%S")
            backup = PRESETS_FILE.with_name(f"{PRESETS_FILE.name}.migration-{stamp}.bak")
            if PRESETS_FILE.exists() and not backup.exists():
                shutil.copy2(PRESETS_FILE, backup)
            preset["audio_path"] = canonical
            _save_json(PRESETS_FILE, _presets)
            logger.info("Persisted migrated reference path for preset %s", preset.get("id"))
    else:
        preset["audio_path"] = canonical
    return resolved
...
def _ref_profile(preset: dict) -> Optional[tuple[int, float]]:
    ...
    """
    try:
        duration_s = sf.info(_reference_path(preset)).duration
    except Exception:
        logger.warning("Could not measure reference clip %s; using static budget", preset.get("audio_path"))
        return None
    if duration_s <= 0:
        return None
...
def _ref_cache_key(preset: dict) -> Optional[tuple]:
    ...
    """
    try:
        path = _reference_path(preset)
        mtime = path.stat().st_mtime_ns
    except (StoredMediaPathError, OSError):
        return None
...
def _ref_facts(preset: dict) -> _RefFacts:
    ...
    try:
        ref_seconds = round(sf.info(_reference_path(preset)).duration, 1)
    except Exception:
...
def _process_job(job_id: str) -> None:
            ...
            try:
                ...
                with _gen_lock:
                    ...
                    stream = _tts.generate_voice_clone_streaming(
                        text=chunk,
                        language=language,
                        ref_audio=str(_reference_path(preset)),
    ...
    entry = {
        ...
        # Kept although nothing predicts any more: generation_s and
        # total_chunks are MEASURED facts and are the raw material if an
        # estimate is ever attempted again. estimated_s is no longer written --
...
# Overridable via ALLOWED_ORIGINS in services/voice-api/.env (comma-separated).
# The default covers the Vite dev server on the same machine.
ALLOWED_ORIGINS = [
...
def verify_admin_password(req: AdminDeleteRequest):
    _require_admin_password(req.admin_password)
    return {"ok": True}
app.add_middleware(
    CORSMiddleware,
...
def _preset_response(preset: dict) -> dict:
    ...
    facts = _ref_facts(preset)
    try:
        preview_name = _reference_path(preset).name
    ...
    return {
        **preset,
        "preview_url": f"/refs/{preview_name}" if preview_name else None,
...
async def create_preset(
    ...
    preset = {
        ...
        "language": language,
        "ref_text": ref_text,
        "audio_path": storage_relative_path(dest, STORAGE_DIR),
        "tag": tag,
        "is_builtin": False,
        "created_at": time.time(),
    }
...
def delete_preset(preset_id: str, req: AdminDeleteRequest, user_id: str = Depends(get_current_user)):
    _require_admin_password(req.admin_password)
    preset = _find_preset(preset_id)
    if preset is None or preset.get("user_id") != user_id:
        raise HTTPException(404, "Unknown preset_id")
    with _store_lock:
        ...
        _save_json(PRESETS_FILE, _presets)
    try:
        _reference_path(preset).unlink(missing_ok=True)
    except StoredMediaPathError:
        logger.warning("Deleted preset %s with missing reference clip", preset_id)
    return {"ok": True}
...
def delete_history_entry(entry_id: str, req: AdminDeleteRequest, user_id: str = Depends(get_current_user)):
    _require_admin_password(req.admin_password)
...
def generate(req: GenerateRequest, user_id: str = Depends(get_current_user)) -> GenerateJobStart:
    ...
    return GenerateJobStart(
        job_id=job_id, total_chunks=len(chunks), queue_position=position,
    )
...
# shadow them. Only activates when apps/studio/dist exists (i.e. `npm run build`
# has run) -- in local dev, where Vite's own dev server handles the frontend
# on :5173, apps/studio/dist doesn't exist and this block never registers.
if getattr(sys, "frozen", False):
    ...
    FRONTEND_DIST = Path(__file__).resolve().parents[2] / "apps" / "studio" / "dist"

# Gated on index.html, not on the directory: a half-written apps/studio/dist (Vite
